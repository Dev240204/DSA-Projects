...
// Declaration of Functions performed
void Display(int n);
void SignUp();
void Login();
void Edit();
...
{
    int n;
    system("cls");//clears terminal before running code
    printf("INSTRUCTIONS\n");
    printf("\n");
    ...
    do
    {
        ...
        scanf("%d", &n);
        switch (n)//invoking functions as per user preference
        {
        case 1:
    ...
    } while (n != 5);//if 5 is entered, program will exit
    return 0;
}
// All the function Used in Program
...
void SignUp()
{
    ...
    do
    {
        if (i != count)//comparing count with i to increment count with each data entry
        {
            i = count;
        }
        ...
        count += i;
        printf("\n");
    } while (0);//so loop will run only once 
}
int Check()//will return index if account is found otherwise -1
{
    char usr[20], pas[20];
    int j = 0, k = 0, check = 10, flag;
    int i = 0;
    printf("Enter your Username :");
    scanf("%s", &usr);
    printf("Enter your Password :");
    ...
    while (i < 10)
    {
        for (int i = 0; i < 10; i++)
        {
            ...
            //checking if username length entered by user is same as any  of the existing entries
            {
                for (int j = 0; j < strlen(usr); j++)
                {
                    if (usr[j] != user[i].username[j])//check=1 if username doesn't match at index j
                    {
                        check = 1;
                        break;
                    }
                    ...
                    {
                        flag = i;//when correct username is entered
                    }
                }
            }
            else
            ...
            for (i = 0; i < 10; i++)
            {
                if (strlen(pas) == strlen(user[i].password))
                {
                    for (int k = 0; k < strlen(pas); k++)
                    {
                        ...
                        {
                            flag = i;//when correct password for entered username is entered
                        }
                    }
                }
                else
...
void Login()
{
    ...
    {
        if(delete==1){
            printf("Account has been deleted");//when credentials of deleted account are entered
        }
        printf("Incorrect Username or Password");//when wrong credentials are entered
    }
    else
    {
        Display(loginAcc);//when correct credentials are entered
    }
    printf("\n");
}
void Edit()
{
    ...
    else
    {
        do
        {
            ...
            scanf("%d", &n);
            switch (n)//to edit the detail according to user preference
            {
             case 1:
...
void Delete()
{
    ...
    else
    {
        ...
        {
            user[i] = user[i + 1];//overwrite deleted account with next entry in structure
        }
